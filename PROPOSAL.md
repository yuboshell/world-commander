...
- *M1, StarCraft II, on yubopc.* Stand up LLM-PySC2 with the clock unpaused; the LLM executes a fixed scripted strategy, so the benchmark isolates execution, not strategy. The headline metric is **macro command-following fidelity across the deadline frontier**, not raw win rate (auto-attack-confounded).
