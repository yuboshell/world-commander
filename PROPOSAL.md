...
Metrics. **Macro command-following fidelity** (did the executor build, expand, switch tech, set army composition, and position as ordered) as a function of (decision latency budget, VRAM budget): an efficiency frontier rather than a single score. Macro is the unconfounded target because the built-in auto-attack already handles micro-combat, so fidelity there measures the executor, not the game's autopilot. **Win rate is demoted to secondary context** (auto-attack-confounded and not a clean LLM signal: the observed edge was ~+10 pp and not significant). Also secondary: decision deadline misses, tokens per decision, actions issued per minute.

Strategic-memory probes. Scripted scenarios where winning requires recalling information observed minutes earlier (scouted tech switches, hidden expansions). These convert cache-eviction failures from abstract retrieval misses into observable command-following failures (a forgotten scouted tech switch, a dropped expansion order), and give eviction methods a task-grounded report card no perplexity benchmark provides.

State tokenization module. Game state is a structured, non-text modality: entities, positions, event streams. Following the program of extending byte-pair-style tokenization beyond text ([Graph Tokenization, ICLR 2026](https://www.diaoenmao.com)), and learned discrete world-model tokenizers that already cut tokens-per-step ([IRIS, ICLR 2023](https://arxiv.org/abs/2209.00588); [Δ-IRIS, ICML 2024](https://arxiv.org/abs/2406.19320)), train a tokenizer on StarCraft II replay corpora and measure tokens-per-decision and end-to-end latency at equal command-following fidelity against text serialization. A compact state code shrinks every downstream cost in the stack.

Deliverables: the open-source harness and the two benchmarks, the efficiency-frontier study, and the memory-probe suite.

**Near-term execution (practical staging).** The milestones are on the coarse (discrete-action) track and need no model training: off-the-shelf and pruned models, prompted.

- *Milestone 1, StarCraft II, on yubopc (now).* Stand up LLM-PySC2 with the clock unpaused; the LLM executes a fixed scripted strategy, so the benchmark isolates execution, not strategy. The headline metric is **macro command-following fidelity across the deadline frontier**, not raw win rate (auto-attack-confounded).
...
**2026-07-02 — feasibility of the fine executor (with Xirui Shi 施稀睿 and Ji Yang 杨佶; Ji is not yet a member).** Both argued the fine (continuous-motion) executor is too costly to be feasible in the foreseeable future: shipped games act through discrete actions (move to a position, cast an ability at a target) and let the engine play authored animation clips; a continuous action space multiplies the cost, and supporting heterogeneous character skeletons is expensive even before generation enters. Assessment: the critique matches the program's existing staging (coarse first; the fine executor parked at Phase 3; Motion BPE standalone), and it strengthens that parking. It does not close the long-term question — single-character real-time generation is already demonstrated (MotionLCM ~30 ms; MotionBricks ~2 ms per character) — but crowds × skeleton diversity × one consumer GPU remains open, which is exactly why P5 is parked rather than active. Their constructive suggestion, skeleton **trajectories**, names a natural middle rung on the level-of-detail axis: generate continuous root paths (a few degrees of freedom per character) and let the engine animate along them — far cheaper than full-body motion, adjacent to the coordination layer (P4). Adopted the same day: the detail ladder is discrete actions → trajectories → full-body motion (hub figures updated; trajectories named as P4's continuous output option; the fine executor stays parked).
