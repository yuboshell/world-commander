...
**AlphaStar** — Grandmaster level in StarCraft II using multi-agent reinforcement learning. Vinyals, Babuschkin, Czarnecki et al. (DeepMind). *Nature 2019*. [Nature](https://www.nature.com/articles/s41586-019-1724-z). **[N]**
...
**WorldMemArena** — Evaluating Multimodal Agent Memory Through Action–World Interaction. UCSB. *arXiv 2026*. [arXiv 2605.29341](https://arxiv.org/abs/2605.29341) · [code](https://github.com/UCSB-AI/WorldMemArena). **[N]**
A multimodal, multi-session benchmark (461 long-horizon interactions) that reframes agent memory as an observe → act → feedback → use loop with a four-stage lifecycle (write / maintain / retrieve / use), so failures localize instead of collapsing into one end-of-task accuracy; spans 19 baselines (retrieval and embedding memory, long-context VLMs, terminal-agent harnesses).
*Relevance:* a methodological sibling, not a baseline. It targets *agent-level* memory (external write/store/retrieve pipelines), a different layer from our *KV-cache* budget, but makes our methodological move in another domain: reject the static proxy (end-of-task QA) and score memory through closed-loop interaction. Closest touchpoint: its lifecycle is a template for scoring the command arena's referential-memory stressor ("the one I moved west"). Cite with the two senses of "memory" kept distinct.
...
**MotionBricks** — Scalable Real-Time Motions with Modular Latent Generative Model and Smart Primitives. Wang, Dionne, Rempe, Zhao, Yuan, Luo, Peng, Zhu, Yuen et al. (NVIDIA). *SIGGRAPH 2026 / ACM ToG*. [arXiv 2604.24833](https://arxiv.org/abs/2604.24833) · [project](https://nvlabs.github.io/motionbricks/) · [code](https://github.com/NVlabs/GR00T-WholeBodyControl). **[N]**
A single modular latent generative backbone spanning 350k+ motion skills (trained on the BONES-SEED mocap set) plus "smart primitives" — a control interface for navigation targets, interaction keyframes, and style — generating motion in real time (reported ~15k FPS, 2 ms) with no per-task fine-tuning; shipped inside NVIDIA's GR00T whole-body humanoid control.
*Relevance:* the strongest current per-character real-time motion backbone, and a graphics-side instance of the slow-backbone / fast-primitive (π0-style) split. Its gap is exactly our Phase-3 question: it is single-character and control-driven (velocity / keyframe / style, not natural language); language-commanded *crowds* under one GPU's budget is what we add.
